...
def load_last_booking():
    ...
    try:
        ...
        print(f"Error reading booking history: {e}")
    return None
...
def append_booking_change(count, timestamp):
    ...
    if os.path.exists(BOOKING_HISTORY_FILE):
        try:
            ...
            print(f"Error reading booking history for append: {e}")
            data = []
    data.append(entry)
    try:
        ...
        print(f"Error writing booking history: {e}")
    ...
    try:
        ...
        for label in soup.find_all("label", class_="checkbox_container"):
            if "Available to book" in label.get_text():
                print(f"Found 'Available to book' label: {label.get_text()}")
                match = re.search(r"\\((\\d+)\\)", label.get_text())
                if match:
                    number = int(match.group(1))
                    print(f"Number of available bookings: {number}")
                else:
                    print("No available bookings found.")
                    number = 0

                return number
                break
        print("'Available to book' label not found or no number present.")
        return None
    ...
    while True:
        try:
            ...
            if count is not None:
                print(f"Available rentals for selected filters: {count}")
                if last_count is not None and count != last_count:
                    print(f"Number changed from {last_count} to {count}!")
                    send_email(
                ...
                print("Could not find rental count.")
        except Exception as e:
